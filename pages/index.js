...
const GlobalStyle = () => (
  <style>{`
    ...
    * { box-sizing: border-box; }
    a.guide-card:hover { border-color: #5aacff !important; }
  `}</style>
);
...
const REASONS = [
  { id: 'deadline', label: 'Landlord missed the legal return deadline', sub: 'Most common winning ground — tribunals rule in tenant favour automatically' },
  { id: 'wearandtear', label: 'Deductions for normal wear and tear', sub: 'Illegal in all provinces — scuffs, fading, minor carpet wear are not chargeable' },
  { id: 'noproof', label: 'Deductions with no receipts or proof', sub: 'Landlord must provide invoices — LTB T1 rulings consistently reject undocumented claims' },
  { id: 'noitemized', label: 'No itemized list of deductions provided', sub: 'Required by law in all provinces — failure forfeits right to deduct' },
  { id: 'partial', label: 'Deposit partially returned without explanation', sub: 'Partial return with no statement is treated same as full withholding by tribunals' },
  { id: 'falsedamage', label: 'Landlord claims damage that was pre-existing', sub: 'Move-in inspection reports and photos are key — document everything' },
  { id: 'cleaning', label: 'Charged for professional cleaning despite normal cleanliness', sub: 'RTB and LTB rulings: landlord cannot charge cleaning fees unless unit was left in extreme condition' },
  { id: 'noinspection', label: 'No move-out inspection was conducted', sub: 'In BC, AB and others: failure to conduct inspection forfeits right to claim damages' },
];
...
const PROVINCE_GUIDES = [
  { code: 'ON', name: 'Ontario', href: '/ontario', hook: 'Damage deposits are illegal here.', law: 'Residential Tenancies Act, 2006', badge: 'Most searched', badgeColor: '#5aacff' },
  { code: 'BC', name: 'British Columbia', href: '/bc', hook: 'Miss the 15-day window — landlord owes double.', law: 'Residential Tenancy Act (BC)', badge: 'Highest penalties', badgeColor: '#34d399' },
  { code: 'AB', name: 'Alberta', href: '/alberta', hook: '10-day return rule. Easy RTDRS filing.', law: 'Residential Tenancies Act (AB)', badge: null, badgeColor: null },
  { code: 'QC', name: 'Quebec', href: '/quebec', hook: 'All security deposits are completely illegal.', law: 'Civil Code of Québec, Art. 1904', badge: 'Unique rules', badgeColor: '#f59e0b' },
];
...
export default function DepositBack() {
  ...
  const generateLetter = async (overrideData) => {
    ...
    if (!payload.fullName) payload.fullName = [payload.firstName, payload.middleName, payload.lastName].filter(Boolean).join(' ');
    if (!payload.fullAddress) payload.fullAddress = [payload.unit ? 'Unit ' + payload.unit + ',' : '', payload.streetAddress, payload.city, payload.province, payload.postalCode].filter(Boolean).join(' ');
    try {
  ...
  const downloadPDF = () => {
    ...
    script.onload = () => {
      ...
      const dw = doc.getTextWidth('Deposit');
      doc.setTextColor(42, 106, 191);
      doc.text('Back', margin + dw, 52);
      doc.setDrawColor(26, 42, 94); doc.setLineWidth(1.5);
  ...
  const restart = () => {
    setStep('landing'); setForm(EMPTY); setLetter(''); setEditedLetter('');
    setEditing(false); setError(''); localStorage.removeItem('db_form');
  };
  ...
  if (step === 'landing') return (
    <div style={s.page}>
      ...
      <div style={s.guidesWrap}>
        <p style={s.sectionTitle}>Know Your Rights by Province</p>
        <p style={s.guidesSub}>Every province has different rules — and different deadlines. Read the guide for yours, then generate your letter.</p>
        <div style={s.guidesGrid}>
          {PROVINCE_GUIDES.map(g => (
            <a key={g.code} href={g.href} className="guide-card" style={s.guideCard}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
                <span style={s.guideCode}>{g.code}</span>
                {g.badge && (
                  <span style={{fontSize:10,fontWeight:700,padding:'2px 8px',borderRadius:20,border:'1px solid',letterSpacing:'0.5px',color:g.badgeColor,borderColor:g.badgeColor+'55',background:g.badgeColor+'18'}}>
                    {g.badge}
                  </span>
                )}
              </div>
              <div style={s.guideName}>{g.name}</div>
              <div style={s.guideHook}>"{g.hook}"</div>
              <div style={s.guideLaw}>{g.law}</div>
              <div style={s.guideArrow}>Read the guide →</div>
            </a>
          ))}
        </div>
      </div>
      ...
      <div style={s.priceWrap}>
        ...
        <div style={s.priceCard}>
          ...
          <ul style={s.features}>
            {['✓  AI-written, province-specific letter','✓  Legally grounded dispute reasons','✓  Editable before you send','✓  Download as PDF — ready to email','✓  All 10 provinces covered'].map(feat=><li key={feat} style={s.feat}>{feat}</li>)}
          </ul>
          <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:8}}>
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              <span style={{background:'#ff4d4d',color:'#fff',fontSize:11,fontWeight:700,padding:'3px 8px',borderRadius:4,letterSpacing:'1px'}}>30% OFF</span>
      ...
      <footer style={s.footer}>
        <div style={s.footerInner}>
          <div>
            <div style={s.footerLogo}>Deposit<span style={s.blue}>Back</span></div>
            <div style={{fontSize:11,color:'#3d6480',marginTop:6,maxWidth:200,lineHeight:1.5}}>Canadian tenant demand letters. Province-specific. Instant.</div>
          </div>
          <div style={s.footerCols}>
            <div style={s.footerCol}>
              <div style={s.footerColTitle}>Province Guides</div>
              {PROVINCE_GUIDES.map(g=>(
                <a key={g.code} href={g.href} style={s.footerLink}>{g.name}</a>
              ))}
            </div>
            <div style={s.footerCol}>
              <div style={s.footerColTitle}>Get Started</div>
              <button onClick={()=>setStep('form')} style={s.footerBtn}>Generate a Letter</button>
            </div>
          </div>
        </div>
        <div style={s.footerBottom}>
          © {new Date().getFullYear()} DepositBack.ca · For informational purposes only · Not legal advice
        </div>
      </footer>
    </div>
  );
      ...
      <div style={s.formPage}>
        <div style={s.formWrap}>
          ...
          <div style={s.row}>
            ...
            <div style={s.field}><label style={s.label}>Postal Code</label><input style={s.input} placeholder='M5V 1A1' value={form.postalCode} maxLength={7}
              onChange={e=>{let v=e.target.value.toUpperCase().replace(/[^A-Z0-9]/g,'');if(v.length>3)v=v.slice(0,3)+' '+v.slice(3);f('postalCode',v);}} /></div>
          </div>
          ...
          <div>
            {REASONS.map(r=>(
              <button key={r.id} style={{...s.reason,...(form.reasons.includes(r.id)?s.reasonActive:{})}} onClick={()=>toggleReason(r.id)}>
                ...
                <div><div style={{fontWeight:600,marginBottom:3}}>{r.label}</div><div style={{fontSize:11,opacity:.7,lineHeight:1.4}}>{r.sub}</div></div>
              </button>
            ))}
          </div>
          ...
          <div style={s.field}>
            <label style={s.label}>Your situation in your own words <span style={s.optional}>(optional)</span></label>
            <textarea style={s.textarea}
              ...
              value={form.extraDetails} onChange={e=>f('extraDetails',e.target.value)} />
          ...
          {isValid && (
            <div style={s.summary}>
              ...
              {[['Tenant',fullName],['Province',pv?.label],['Property',fullAddress],['Landlord',form.landlordName],['Disputed','$'+withheld.toFixed(2)+' CAD'],['Price','$6.99 CAD']].map(([k,v])=>(
    ...
    <div style={s.page}>
      ...
      <div style={s.formPage}>
        <div style={{maxWidth:740,margin:'0 auto'}}>
          ...
          {editing?(
            <textarea style={{...s.letterBox,color:'#1a2a3a',resize:'vertical',minHeight:500,outline:'none',cursor:'text',border:'2px solid #5aacff',width:'100%',boxSizing:'border-box',fontFamily:"Georgia,serif",fontSize:13,lineHeight:1.9}}
          ...
          <div style={s.nextBox}>
            ...
            {['Send by email — it creates a timestamped paper trail','CC yourself so you have proof of delivery','Follow up with a printed copy if no response in 7 days','If ignored after 14 days, file with your provincial tribunal — free to do'].map(t=>(
...
const styles = {
  ...
  // Province guides
  guidesWrap:{maxWidth:720,margin:'0 auto',padding:'0 24px 56px'},
  guidesSub:{fontSize:13,color:'#3d6480',textAlign:'center',marginBottom:28,marginTop:-18,lineHeight:1.6},
  guidesGrid:{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:12},
  guideCard:{background:'#0e1f36',border:'1px solid #1b3454',borderRadius:10,padding:'20px',textDecoration:'none',color:'inherit',display:'block'},
  guideCode:{fontSize:10,fontWeight:700,color:'#3d6480',fontFamily:'monospace',letterSpacing:'1.5px'},
  guideName:{fontSize:16,fontWeight:700,color:'#fff',marginBottom:8},
  guideHook:{fontSize:12,color:'#8ab4d4',fontStyle:'italic',marginBottom:10,lineHeight:1.5},
  guideLaw:{fontSize:10,color:'#3d6480',fontFamily:'monospace',marginBottom:14},
  guideArrow:{fontSize:12,color:'#5aacff',fontWeight:600},
  // Pricing
  ...
  // Footer
  footer:{borderTop:'1px solid #1b3454'},
  footerInner:{maxWidth:720,margin:'0 auto',padding:'40px 24px 32px',display:'flex',justifyContent:'space-between',alignItems:'flex-start',flexWrap:'wrap',gap:32},
  footerLogo:{fontSize:18,fontWeight:700,color:'#fff'},
  footerCols:{display:'flex',gap:48,flexWrap:'wrap'},
  footerCol:{display:'flex',flexDirection:'column',gap:10},
  footerColTitle:{fontSize:10,letterSpacing:'2px',textTransform:'uppercase',color:'#3d6480',fontFamily:'monospace',marginBottom:2},
  footerLink:{fontSize:13,color:'#8ab4d4',textDecoration:'none'},
  footerBtn:{background:'none',border:'none',fontSize:13,color:'#8ab4d4',cursor:'pointer',fontFamily:'inherit',padding:0,textAlign:'left'},
  footerBottom:{borderTop:'1px solid #1b3454',padding:'16px 24px',textAlign:'center',fontSize:11,color:'#3d6480'},
  // Form
